...
class HenrySBA():
    def __init__(self):
        ...
        self.book_combo.bind("<<ComboboxSelected>>", HenrySBA.book_selection) # Bind a callback
...
class HenrySBP():
    ...
    # Get info when the publisher is changed and autofill the first book information
    def publisher_selection(event):
        selIndex = event.widget.current()
        pub = henryDB.publisher_list[selIndex]
        henryDB.getBooksByPublisher(pub.pub_code) # Getting the correct publisher code
        ...
        # Book info for the selected publisher
        ...
        # Filling in the book combo box and correct price
        henrySBP.book_combo['values'] = henrySBP.book_list  # Book combobox is updated with publisher's books
...
class HenrySBC():
    ...
    def category_selection(event):
        ...
        # Book info for the selected category
...
henrySBC = HenrySBC()
root.mainloop()
henryDB.close() # Close the connection to the database
